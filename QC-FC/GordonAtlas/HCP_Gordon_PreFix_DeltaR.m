...
rt='/Volumes/HCP_S900/HCP_100Unrelated/PreFix_NetMats/Gordon/NetMats';

cnt_s=1;
for s=SubList
    
    disp([num2str(cnt_s) ' ' s{1} '  ------'])
    load([rt '/HCP_PreFix_' s{1} '_Gordon_NetMats_DVC.mat'],'ED','mat','mat_dvr','FDts','DVARS','DSE_Stat','mat_dvs_scrmbld')
    ...
    hf0=figure('position',[50,500,700,300]); hold on; box on; 
    ...
    disp(['dens of affected: ' num2str(sum(H)/(nn*(nn-1)/2))]); %number of sig edges
    ...
    set(gcf,'Color','w')
    %export_fig(hf0,[s{1} '_Gordon_PreFix_DeltaR.png']);
